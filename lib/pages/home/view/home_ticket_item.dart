...
class KKHomeTicketItem extends StatefulWidget {
  ...
  const KKHomeTicketItem(@required this.bgImageStr,
      ...
      @required this.tickInfo, {this.socketMap ,super.key});
...
class _KKHomeTicketItemState extends State<KKHomeTicketItem> {
  ...
  void initState() {
    ...
    super.initState();
    if(widget.socketMap == null) {
      periodsNumber = widget.tickInfo["current"]["periodsNumber"].toString();
      endTime = widget.tickInfo["current"]["autoCloseDate"];
      isShowStatus = widget.tickInfo["current"]["status"] != 4;
    }else {
      periodsNumber = widget.socketMap!["periodsNumber"];
      endTime = int.parse(widget.socketMap!["autoCloseDate"]);
      isShowStatus = widget.socketMap!["status"] != 4;
    ...
    return Column(
      children: [
        ...
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.center,
          children: widget.playList.asMap().entries.map((e){
            return _buildOddsItem(e.value as Map);
          }).toList(),
        ),
        const SizedBox(height: 20,),
        Row(
  ...
  Widget _buildOddsItem(Map playInfo) {
    bool isPCNN = playInfo["lotteryCode"] == "PCNN";
    String content = "";
    if(isPCNN) {
      Map pcnnInfo = (playInfo["cachePlayList"] as List).first;
      content = "${pcnnInfo["playName"]}:${pcnnInfo["odds"]}-${pcnnInfo["maxOdds"]}";
    }else {
  ...
  void startEndTime() {
    ...
    DateTime haveTime = DateTime.fromMillisecondsSinceEpoch(endTime*1000-time, isUtc: true);
