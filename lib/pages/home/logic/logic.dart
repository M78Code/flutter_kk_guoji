...
import 'package:kkguoji/services/event_bus_util.dart';
...
class HomeLogic extends GetxController {
  ...
  final itemLogic = Get.find<ItemLogic>();
  ...
  getPopupNotice() async {
    var result =
        await HttpRequest.request(HttpConfig.getPopupNotice, method: "get");
    if (result["code"] == 200) {
      String dataStr = formatDate(DateTime.now(), [yyyy, "-", mm, "-", dd]);
      bool? isHidden = sqliteService.getBool(dataStr);
      if (isHidden == null) {
        Get.dialog(NoticeWidget((result["data"] as List).first),
  ...
  getRecommendGameList() async {
    ...
    if (gameResult["code"] == 200) {
      ...
      if (recommendGameModel.data.list != null) {
        ///数据重新排序
        List<RecommendList> newList =
            sortDataList(recommendGameModel.data.list, 3);
  ...
  updateTicketInfo() {
    ...
    JcpGameSocketModel socketModel =
        JcpGameSocketModel.fromJson(ticketInfo.value.values.first);
    ...
    if (item != null) {
      ...
      // Future.delayed(const Duration(seconds: 5), () {
        ...
        item.current?.autoCloseDate =
            num.parse(socketModel.autoCloseDate ?? '0');
        item.current?.autoDrawingDate =
            num.parse(socketModel.autoDrawingDate ?? '0');
        margeData();
        // eventBus.sendEvent('$kChangeMainPageEvent${item?.lotteryCode}');
      // });
    }
    eventBus.sendEvent('$kChangeMainPageEvent${item?.lotteryCode}');
    // gameList.refresh();
  ...
  void openGame() async {
    ...
    var result =
        await HttpRequest.request(HttpConfig.getGameByCompanyCode, params: map);
    if (result["code"] == 200) {
  ...
  void loginSportGame(Map gameMap, int gameId) async {
    ...
    if (result["code"] == 200) {
      RouteUtil.pushToView(Routes.webView, arguments: [
        result["data"]["url"],
        gameId,
        gameMap["gamePlatformName"]
      ]);
    }
  }
  ...
  void loginGame(Map gameMap) async {
    Map gameInfo = gameMap.values.first;
    Map<String, dynamic> params = {"game_id": gameInfo["id"],"platform":"h5"};
    var result = await HttpRequest.request(HttpConfig.loginGame, method: "post", params: params);
    if (result["code"] == 200) {
      RouteUtil.pushToView(Routes.webView, arguments: [
        result["data"]["url"],
  ...
  gamesOnTap(String type, String lotteryId) async {
    GetGameModel? getGameModel =
        await GamesApi.getGameByCompanyCode(type, lotteryId);
    GameLogin? gameLogin = await GamesApi.gameLogin(
        getGameModel?.gameCompanyCode ?? "",
        (getGameModel?.id ?? "").toString());
    if (gameLogin?.url != null) {
      print('加载第三方url；${gameLogin?.url}');
      RouteUtil.pushToView(Routes.webView, arguments: [
        gameLogin?.url ?? "",
        getGameModel?.id,
        getGameModel?.gamePlatformName
      ]);
    }
  }
  ...
  margeData() {
    ...
    print(
        'xiaoan 首页已合并数据：${gameList.length} ${margeGameList.length} ${JsonUtil.encodeObj(margeGameList)}');
  }
