...
class AccountApi {
  ...
  static Future<UserInfoModel?> uploadImage(String imgPath, {FormData? formData}) async {
    final data = formData ??
        FormData.fromMap({
          "type": "image/*",
    ...
    var response = await HttpService.uploadImage(HttpConfig.uploadImage, data);
  ...
  static Future<bool> updateUserInfo(Map<String, dynamic> params) async {
    final response = await HttpService.request(HttpConfig.updateUserInfo, method: "POST", params: params);
  ...
  static Future<UserMoneyDetailsSearchListModel?>? getUserMoneyDetailsSearch(String dateType, int page, String type) async {
    var result = await HttpRequest.request(HttpConfig.getUserMoneyDetailsSearch, method: "post", params: {"dateType": dateType, "page": page, "limit": 20, "type": type});
    if (result["code"] == 200) {
  ...
  static Future<UserWithdrawListModel?>? getUserWithdrawList(String time_range, int page, String type) async {
    var result = await HttpRequest.request(HttpConfig.getUserWithdrawList, method: "get", params: {"time_range": time_range, "page": page, "limit": 20});
    if (result["code"] == 200) {
  ...
  // yesterday=昨日，today=今日，last_week=上周，week=本周，month=本月，last_month=上月 Y-m-d - Y-m-d 自定义范围
  static Future<UserRechargeListModel?>? getUserRechargeList(String time_range, int page, String type) async {
    var result = await HttpRequest.request(HttpConfig.getUserRechargeList, method: "get", params: {"time_range": time_range, "page": page, "limit": 20});
    if (result["code"] == 200) {
